...
const MenuPage = () => {
  ...
  // Group by category
  const categories = ["ice-cream", "pizza", "burger", "sandwich", "soft-drinks"];
          ...
          <div className="flex gap-6 overflow-hidden pb-4 scrollbar-hide">
            {getItemsByCategory(cat).map((item) => (
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
                className="min-w-[250px] bg-white/40 backdrop-blur-md rounded-xl shadow-lg p-4 border border-white/30 flex flex-col"
              >
                <img
                  src={item.image}
                  alt={item.name}
